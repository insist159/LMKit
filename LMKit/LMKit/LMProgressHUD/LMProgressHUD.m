...
#define LMMainThreadAssert() NSAssert([NSThread isMainThread], @"LMProgressHUD needs to be accessed on the main thread.");

CGFloat const LMProgressMaxOffset = 1000000.f;

static const CGFloat LMDefaultPadding = 4.f;
static const CGFloat LMDefaultLabelFontSize = 16.f;
static const CGFloat LMDefaultDetailsLabelFontSize = 12.f;
...
@interface LMProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    LMProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    LMProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (LMProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            LMProgressHUD *hud = (LMProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = LMProgressHUDAnimationFade;
    _mode = LMProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(LMProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == LMProgressHUDAnimationZoom) {
        type = animatingIn ? LMProgressHUDAnimationZoomIn : LMProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == LMProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == LMProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == LMProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == LMProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    LMProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<LMProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    LMBackgroundView *backgroundView = [[LMBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = LMProgressHUDBackgroundStyleSolidColor;
    ...
    LMBackgroundView *bezelView = [LMBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:LMDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:LMDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [LMProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:LMDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[LMRoundProgressView class]];

    LMProgressHUDMode mode = self.mode;
    if (mode == LMProgressHUDModeIndeterminate) {
    ...
    else if (mode == LMProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[LMBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LMProgressHUDModeDeterminate || mode == LMProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[LMRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == LMProgressHUDModeAnnularDeterminate) {
            [(LMRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == LMProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LMProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[LMProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[LMProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[LMRoundProgressView class]]) {
        LMRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LMRoundProgressView appearanceWhenContainedIn:[LMProgressHUD class], nil];
#else
        appearance = [LMRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[LMProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((LMRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((LMRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[LMBarProgressView class]]) {
        LMBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LMBarProgressView appearanceWhenContainedIn:[LMProgressHUD class], nil];
#else
        appearance = [LMBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[LMProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((LMBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((LMBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    LMBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? LMDefaultPadding : 0.f;
...
- (void)setMode:(LMProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == LMProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation LMRoundProgressView
...
@implementation LMBarProgressView
...
@implementation LMBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = LMProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(LMProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    LMProgressHUDBackgroundStyle style = self.style;
    if (style == LMProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == LMProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation LMProgressHUDRoundedButton
